...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    // https://stackoverflow.com/questions/10428629/programmatically-set-the-initial-view-controller-using-storyboards
        ...
        func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
            ...
            let hasAgreed = defaults.bool(forKey: "hasAgreedToEula")
            
            // User has not agreed
            if (hasAgreed) {
                // User has agreed
                window?.rootViewController = storyboard.instantiateViewController(withIdentifier: "agree")
            } else {
                window?.rootViewController = storyboard.instantiateViewController(withIdentifier: "Eula")
            }
